...
[![CI/CD](https://github.com/backstage-learning-durelli/gerador-de-planilha/actions/workflows/ci-cd.yaml/badge.svg)](https://github.com/backstage-learning-durelli/gerador-de-planilha/actions/workflows/ci-cd.yaml)
...
```json
{
  "message": "Hello from gerador-de-planilha!"
}
```
...
```bash
# Build da imagem
docker build -t gerador-de-planilha:latest .

# Executar container
docker run -p 8000:8000 gerador-de-planilha:latest
...
4. ✅ Publica no Docker Hub (`durellirsd/gerador-de-planilha`)
...
- **Application**: `gerador-de-planilha`
- **Namespace**: `gerador-de-planilha`
- **Sync Policy**: Automático (prune + self-heal)

**Acessar no ArgoCD:**
```bash
# Via CLI
argocd app get gerador-de-planilha
argocd app sync gerador-de-planilha

# Ver logs
kubectl logs -n gerador-de-planilha -l app=gerador-de-planilha -f
```
...
kubectl get all -n gerador-de-planilha

# Acessar a aplicação (port-forward)
kubectl port-forward -n gerador-de-planilha svc/gerador-de-planilha 8000:80
